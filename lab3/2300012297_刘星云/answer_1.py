...
def integration(x:np.ndarray, R:np.ndarray, v:np.ndarray, w:np.ndarray, h:float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    ####### 你的回答 #######
    x_next = x + h * v
    delta_rot = Rotation.from_rotvec(w * h) 
    R_current = Rotation.from_matrix(R)
    R_updated = delta_rot * R_current 
    R_next = R_updated.as_matrix() 
...
def forward_dynamics(
    ...
    Iw = np.einsum('bij,bj->bi', I_world, w)         
    w_cross_Iw = np.cross(w, Iw)                
    torque_eff = tau - w_cross_Iw                  
    dw = np.einsum('bij,bj->bi', inv_I_world, torque_eff)  
    w_next = w + h * dw
    #######################
