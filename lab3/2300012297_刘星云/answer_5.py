...
def PD_control(x:np.ndarray, R:np.ndarray, v:np.ndarray, w:np.ndarray, joints:List[Joint],
    ...
    ######## 你的回答 #######
    kp = np.ones(num_joints) * kp if np.isscalar(kp) else kp
    kd = np.ones(num_joints) * kd if np.isscalar(kd) else kd

    for j in range(num_joints):
        R_current = R_bodyA[j].T @ R_bodyB[j]
        R_target = target_joint_rotations[j]
        R_error = R_target @ R_current.T
        rotvec = Rotation.from_matrix(R_error).as_rotvec() 
        rotvec_world = R_bodyA[j] @ rotvec

        # 角速度误差
        w_err = w_bodyB[j] - w_bodyA[j]

        # PD 力矩
        tau = kp[j] * rotvec_world - kd[j] * w_err
        joint_torques[j] = tau
