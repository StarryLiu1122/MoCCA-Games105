...
> 在 MacOS 上，如果运行遇到 `ModuleNotFoundError: No module named '_tkinter'` 或类似错误，你可能需要安装 python-tk。可以使用 homebrew 安装，命令 `brew install python-tk`。关于如何安装 homebrew，以及如何安装对应 python 版本的 python-tk，请自行搜索解决。

> 目前暂不提供 linux 版本的预编译库，如需要可以自行编译安装。下载 MoCCASimuBackend-update.zip，并在解压后文件夹中使用 `pip install .` 命令来进行安装。安装遇到问题，请自行搜索解决。
